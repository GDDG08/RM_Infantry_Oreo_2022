...
void PowerCtrl_Init(Power_ControlModeMnum Ctrl_state, Motor_MotorGroupTypeDef* Mecanum) {
    ...
    PowCtr->PowerCtrl_Ver = POWER_VER_MODERN;
...
void PowerPID_Cal(float Powerref, float Powerfdb) {
    ...
    if (Powerfdb > Powerref - 1.0f) {
        PowCtr->Power_scale = PowCtr->Power_pid.output;
    } else if (Powerfdb > Powerref - 5.0f) {
        PowCtr->Power_scale = PowCtr->Power_pid.output / (Powerref - Powerfdb);
    } else {
...
void PowerCtrl(void) {
    ...
    if (PowCtr->PowerCtrl_State == POWER_UNLIMITED) {
        ...
        } else if (capctrl->cap_mode_Starting == 1 && chassis->mode != Chassis_MODE_GYRO && chassis->mode != Chassis_MODE_SUPERGYRO) {
...
void Power_InitPower(Power_ControlModeMnum mode) {
    ...
    PowerCtrl_Data.PowerCtrl_State = mode;
}
...
void Power_PowerControl(Motor_MotorGroupTypeDef* chassis) {
    if (PowerCtrl_Data.PowerCtrl_State != POWER_LIMIT) {
        return;
    }
